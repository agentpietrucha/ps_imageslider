			...
			{foreach from=$slides item=slide}
				<div id="slides_{$slide.id_slide}" class="panel list-group-item" data-id="{$slide.id_slide}">
					<div class="row">
						...
						<div class="col-md-3">
							{assign var=file_type value="."|explode:$slide.image_desktop|@end}
							{if $file_type|in_array:['mp4']}
								<video class="img-thumbnail" {if $slide.poster_desktop}poster="{$image_baseurl}{$slide.poster_desktop}"{/if} controls autoplay playsinline muted>
									<source type="video/mp4" src="{$image_baseurl}{$slide.image_desktop}" alt="{$slide.title}"/>
								</video>
							{else}
